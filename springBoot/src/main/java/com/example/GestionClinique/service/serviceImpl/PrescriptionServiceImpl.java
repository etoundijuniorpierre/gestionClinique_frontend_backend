...
public class PrescriptionServiceImpl implements PrescriptionService {
    ...
    public Prescription addPrescription(Long consultationId, Prescription prescription) {
        ...
        historiqueActionService.enregistrerAction(String.format("Ajout prescription ID: %d pour consultation ID: %d", savedPrescription.getId(), consultationId), loggingAspect.currentUserId());
        return savedPrescription;
    }

    @Override
    public Prescription updatePrescription(Long id, Prescription prescriptionDetails) {
        Prescription existingPrescription = prescriptionRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Prescription not found with ID: " + id));
        ...
        if (prescriptionDetails.getConsultation() != null && !prescriptionDetails.getConsultation().getId().equals(existingPrescription.getConsultation().getId())) {
            Consultation newConsultation = consultationRepository.findById(prescriptionDetails.getConsultation().getId()).orElseThrow(() -> new IllegalArgumentException("Consultation not found with ID: " + prescriptionDetails.getConsultation().getId()));
        ...
        if (prescriptionDetails.getMedecin() != null && !prescriptionDetails.getMedecin().getId().equals(existingPrescription.getMedecin().getId())) {
            Utilisateur newMedecin = utilisateurRepository.findById(prescriptionDetails.getMedecin().getId()).orElseThrow(() -> new IllegalArgumentException("Medecin not found with ID: " + prescriptionDetails.getMedecin().getId()));
        ...
        if (prescriptionDetails.getPatient() != null && !prescriptionDetails.getPatient().getId().equals(existingPrescription.getPatient().getId())) {
            Patient newPatient = patientRepository.findById(prescriptionDetails.getPatient().getId()).orElseThrow(() -> new IllegalArgumentException("Patient not found with ID: " + prescriptionDetails.getPatient().getId()));
        ...
        if (prescriptionDetails.getDossierMedical() != null && !prescriptionDetails.getDossierMedical().getId().equals(existingPrescription.getDossierMedical().getId())) {
            DossierMedical newDossierMedical = dossierMedicalRepository.findById(prescriptionDetails.getDossierMedical().getId()).orElseThrow(() -> new IllegalArgumentException("DossierMedical not found with ID: " + prescriptionDetails.getDossierMedical().getId()));
            existingPrescription.setDossierMedical(newDossierMedical);
        }

        historiqueActionService.enregistrerAction(String.format("Mise à jour prescription ID: %d", id), loggingAspect.currentUserId());
    ...
    public void deletePrescription(Long id) {
        Prescription prescription = prescriptionRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Prescription not found with ID: " + id));
        historiqueActionService.enregistrerAction(String.format("Suppression prescription ID: %d", id), loggingAspect.currentUserId());
